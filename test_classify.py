"""Test the new classification logic with real OCR data."""
import fitz
...
for pn in range(len(doc)):
    ...
    all_scores = {name: s for name, s in details.items() if s}
    print(f'Page {pn+1:2d}: {cat:10s} (score={score:3d}) | {details.get(cat, [])}')
...
for candidate in ["付款意见书", "合同文件", "检测报告", "支付申请表", "支付情况统计表"]:
    ...
    for pn in range(min_page, max_page + 1):
        if page_results[pn] != candidate and page_results[pn] != "未分类":
